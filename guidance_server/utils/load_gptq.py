...
def load_gptq_model(model_path: str, general_settings: settings.GeneralSettings, gptq_settings: settings.GPTQSettings):
    ...
    if general_settings.cpu_offloading:
        ...
        model = load_quant_with_offload(model_path, checkpoint, wbits=gptq_settings.wbits, groupsize=gptq_settings.group_size, pre_layer=gptq_settings.pre_loaded_layers)

    else:
        print("Skipping CPU Offloading option. Please try enabling it you run into out of memory issues.")
